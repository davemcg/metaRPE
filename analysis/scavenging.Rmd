...
metadata_abca4KO <- read_tsv('../data/metadata_abca4KO.tsv')
...
dominik_meta <- readxl::read_xlsx('../data/dominik_WNT_iRPE_2021/Layout experiment.xlsx') %>% 
  rename(Sample = `Sample name`,
         cell_line = Cells) %>% 
  mutate(Class = case_when(Treatment == 'CTRL' ~ 'Control',
                           TRUE ~ 'Perturbed'),
         Owner = 'Dominik',
         CellType = 'iRPE')
...
write_csv(ogvfb_meta %>% filter(Class == 'Control', ), file = '../data/control_meta.csv')
write_csv(ogvfb_meta %>% filter(Class == 'Control'), file = '../data/control_meta.csv')

write_csv(ogvfb_meta %>% filter(Class != 'Control'), file = '../data/perturbed_meta.csv')
write_csv(ogvfb_meta %>% filter(Class != 'Control'), file = '../data/perturbed_meta.csv')
```
