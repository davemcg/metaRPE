...
degeneration
# sbatch commands
degeneration %>% mutate(sbatch = glue::glue("sbatch --mem=35G --cpus-per-task 6 --time=8:00:00 run_recount.sh {lane_sample} {Owner}")) %>% pull(sbatch) %>% writeLines()
```

# Recount meta
```{r}
out <- bind_rows(ctrls, degeneration) %>% select(Owner, lane_sample)
