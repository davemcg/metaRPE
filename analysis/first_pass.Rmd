...
# set XY ratio
colData(rse_gene_select)$XYratio <- (colData(dds)$"recount_qc.aligned_reads%.chrx" + 0.01) / (colData(dds)$"recount_qc.aligned_reads%.chry" + 0.01)

# make merged cell line / owner field
colData(rse_gene_select)$CL_Owner <- paste(colData(dds)$cell_line, colData(dds)$Owner, sep = '_')

# load in gff
gff <- rtracklayer::import.gff('../recount3/RSE/human/annotations/gene_sums/human.gene_sums.G029.gtf.gz') %>% as_tibble()
```
...
```{r}
res %>% as_tibble(rownames = 'gene_id') %>% left_join(gff) %>% filter(!is.na(padj)) %>% arrange(padj) %>% DT::datatable()
```
...
volcano_maker(df =res %>% as_tibble(rownames = 'gene_id') %>% left_join(gff) %>% mutate(Gene = gene_name)) + coord_cartesian(xlim = c(-5,5))
```
